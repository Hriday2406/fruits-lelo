...
| -------- | --------------------------------------------------- | ---------------------------------- |
| High     | Add not-found route + resilient product lookup      | Prevent runtime errors on bad URLs |
| High     | Centralize storage utils & validate JSON parse      | Stability & maintainability        |
| High     | Implement No Results message (Issue #3)             | UX clarity                         |
| High     | Make layout responsive (Issue #6)                   | Mobile usability                   |
| Medium   | Add active nav highlight (Issue #4)                 | Orientation                        |
| Medium   | Extract reusable helpers (isInCart, getFruitBySlug) | DRY principle                      |
| Medium   | Convert price to number                             | Data correctness                   |
| Medium   | Add lazy-loading to images                          | Performance                        |
| Medium   | Replace alert() with AntD message/notification      | UI polish                          |
| Medium   | Add accessibility attributes & roles                | Inclusivity                        |
| Low      | Introduce Vitest + RTL tests                        | Quality baseline                   |
| Low      | Add i18n scaffolding                                | Future scalability                 |
| Low      | Consider removing heavy deps if not critical        | Bundle optimization                |
